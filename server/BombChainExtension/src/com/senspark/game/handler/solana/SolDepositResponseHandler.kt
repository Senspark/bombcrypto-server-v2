...
class SolDepositResponseHandler(
    ...
    private val _tag = "[DEPOSIT_SOL_RESPONSE]"
    ...
    fun handle(result: String) {
        _coroutineScope.scope.launch(Dispatchers.Default) {
            ...
            try {
                ...
                var userName = ""
                var depositType = EnumConstants.DepositType.SOL_DEPOSIT

                // First, get the invoice and check user online status
                val invoice = if (response.invoice.startsWith("DEP")) {
                    // Check deposit chéo, token nạp vào phải là SOL
                    val isValid = response.tokenName == "SOL"
                    if (!isValid) {
                        _logger.log("$_tag User cheat deposit sol: invoice: ${response.invoice} > amount: ${response.amount} > txHash: ${response.txHash}")
                        return@launch
                    }
                ...
                if (response.invoice.startsWith("DEP")) {
                    userName = _userDataAccess.updateSolTransaction(
                        invoice,
                        response.amount,
                        response.txHash,
                        response.tokenName
                    )
                    depositType = EnumConstants.DepositType.SOL_DEPOSIT
                } else if (response.invoice.startsWith("BCD")) {
                    userName = _userDataAccess.updateBcoinSolTransaction(
                        invoice,
                        response.amount,
                        response.txHash,
                        response.tokenName
                ...
                if (controller != null) {
                    ...
                    controller.sendDataEncryption(HandlerCommand.DepositSolResponse, params)
                }
                _logger.error("$_tag Error when deposit in SOL network: ${e.message} UserName: ${controller?.userInfo?.username}")
            }
        }
    }
    ...
    fun parse(data: String): DepositSolResponse {
        val response = _json.decodeFromString<DepositSolResponse>(data)
        return response
    }
}