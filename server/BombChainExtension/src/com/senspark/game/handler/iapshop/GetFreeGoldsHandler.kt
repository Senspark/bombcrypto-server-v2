...
class GetFreeGoldsHandler : BaseEncryptRequestHandler() {
    override val serverCommand: String = SFSCommand.GET_FREE_GOLDS_V2
    ...
    override fun handleGameClientRequest(controller: IUserController, requestId: Int, data: ISFSObject) {
        coroutine.scope.launch(Dispatchers.Default) {
            try {
                ...
                iapShopManager.getFreeRewardByAds(controller, token, EnumConstants.BLOCK_REWARD_TYPE.GOLD)
                sendSuccess(controller, requestId, SFSObject())
            } catch (e: Exception) {
                controller.logger.error("Error when get free gold", e)
