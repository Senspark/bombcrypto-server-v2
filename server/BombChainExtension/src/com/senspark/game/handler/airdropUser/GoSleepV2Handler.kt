...
class GoSleepV2Handler : BaseEncryptRequestHandler() {
    override val serverCommand: String = SFSCommand.GO_SLEEP_V2
    ...
    override fun handleGameClientRequest(controller: IUserController, requestId: Int, data: ISFSObject) {
        ...
        val bbm: Hero =
            bbmController.getHero(id, controller.dataType) ?: return sendError(
                controller,
                requestId,
                ErrorCode.BOMBERMAN_NULL,
                null
            )
        if (!bbm.isActive) {
            controller.logger.error("GoSleepHandler: Bomber man not active = $id ${controller.userName}")
            return sendError(controller, requestId, ErrorCode.BOMBERMAN_ACTIVE_INVALID, null)
        }
        if (bbm.stage == BOMBER_STAGE.SLEEP) {
