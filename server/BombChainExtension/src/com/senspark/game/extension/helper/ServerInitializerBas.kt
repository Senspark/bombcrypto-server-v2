...
import com.senspark.game.extension.schedulers.ExtensionSchedulerBas
import com.senspark.game.handler.bas.DepositBasHandler
import com.senspark.game.handler.bas.BasDepositResponseHandler
 ...
 * Server BAS
 */
class ServerInitializerBas(
    private val _services: GlobalServices
) : IServerInitializer {

    private val _netServices = _services.get<ISvServicesContainer>().get(ServerType.BAS)

    override fun initHandlers(helper: AddRequestHandlerHelper) {
        helper.addRequestHandler(SFSCommand.GET_INVOICE_DEPOSIT_BAS, DepositBasHandler::class.java)
    }
    ...
    override fun initStreamListeners() {
        ...
        if (envManager.useStreamListener) {
            ...
            val basDepositHandler = BasDepositResponseHandler(logger, userDataAccess, usersManager, coroutineScope)
            messenger.listen(StreamKeys.AP_BAS_TRANSACTION) { message ->
                basDepositHandler.handle(message.value)
                false
            }
    ...
    override fun initSchedulers() {
        ExtensionSchedulerBas(_services, _netServices).initialize()
    }
}