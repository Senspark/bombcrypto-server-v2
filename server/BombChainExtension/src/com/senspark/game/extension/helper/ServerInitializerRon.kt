...
import com.senspark.game.extension.schedulers.ExtensionSchedulerRon
import com.senspark.game.handler.ron.DepositRonHandler
import com.senspark.game.handler.ron.RonDepositResponseHandler
 ...
 * Server RON
 */
class ServerInitializerRon(
    private val _services: GlobalServices
) : IServerInitializer {

    private val _netServices = _services.get<ISvServicesContainer>().get(ServerType.RON)

    override fun initHandlers(helper: AddRequestHandlerHelper) {
        helper.addRequestHandler(SFSCommand.GET_INVOICE_DEPOSIT_RON, DepositRonHandler::class.java)
    }
    ...
    override fun initStreamListeners() {
        ...
        if (envManager.useStreamListener) {
            ...
            val ronDepositHandler = RonDepositResponseHandler(logger, userDataAccess, usersManager, coroutineScope)
            messenger.listen(StreamKeys.AP_RON_TRANSACTION) { message ->
                ronDepositHandler.handle(message.value)
                false
            }
    ...
    override fun initSchedulers() {
        ExtensionSchedulerRon(_services, _netServices).initialize()
    }
}