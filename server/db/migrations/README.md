...
| 2026-07-08 | `20260708_120000_add_cross_chain_bridge_server_submit.sql` | Cross-chain bridge, complete feature (supersedes the deleted `20260701_110214_add_cross_chain_bridge.sql`, which only ran on local dev). Adds `cross_chain_bridge_sync` / `_pending` / `_history` tables + `fn_bridge_sync_deposit` / `fn_bridge_request_withdraw` / `fn_bridge_sync_withdraw`; seeds `bridge_deposit_enabled` / `bridge_withdraw_enabled` kill-switch flags. Server-submitted withdraw: `_pending` gains a tx lifecycle (`status`/`tx_hash`/`nonce`/`submitting_at`/`attempts`/`last_error`/`updated_at`) + `fn_bridge_refund` / `fn_bridge_mark_submitted` / `fn_bridge_mark_failed`. Bridge balance lives in `user_block_reward` at `(uid, reward_type ∈ {BCOIN_BRIDGE, SEN_BRIDGE}, type='BP')` |
| 2026-07-24 | `20260724_120000_add_native_deposit.sql` | Add native BNB/POL deposit, withdraw, and in-game spend. `user_native_deposited` mirrors the on-chain wei counters; `config_native_rate` + `fn_native_price` convert BCOIN prices to native at a per-network rate. Spendable balance lives in `user_block_reward` (`BNB_DEPOSITED`/`POL_DEPOSITED`), spent through `fn_sub_user_reward` |
| 2026-08-25 | `20260825_100000_fix_auto_mine_price_partition_scan.sql` | Fix `fn_calculate_package_auto_price` seq-scanning every yearly partition of `logs.user_block_reward`. `DATE(changed_at) BETWEEN ...` hid the partition key from the planner, so a 7-day lookup read 2024–2027 on every call; under concurrency this saturated prod disk (21 piled-up backends, oldest 50 min). Replaced with the exactly-equivalent half-open range `changed_at >= (CURRENT_DATE - 7)::timestamptz AND changed_at < (CURRENT_DATE)::timestamptz`, interpolated as literals so pruning happens at plan time. Metadata-only, safe under load |
| 2026-08-25 | `20260825_100100_index_user_block_reward_save_game.sql` | Partial `(uid, changed_at) WHERE reward_type='BCOIN' AND reason='Save game'` indexes on the 2026/2027 partitions of `logs.user_block_reward`. Written for a maintenance window (game server stopped), so plain `CREATE INDEX`. Offers 2A (index the parent, future partitions inherit -- preferred) vs 2B (2026/2027 only, shorter window). Includes pre-checks, because `schema.sql` no longer reflects production's partitioning |
| 2026-09-04 | `20260904_120000_dynamic_native_rate.sql` | Hands `config_native_rate.native_per_bcoin` over to the BNB/POL price scheduler, which rewrites it from the market (`bcoin_usd / native_usd`) instead of it being hand-maintained. No schema change -- the writer updates the existing column and stamps `modify_date`; this only seeds `native_rate_update_minutes` / `native_rate_max_change_percent`, which the server also defaults to |
