...
export async function requestPublicKey(stateInit: string): Promise<Buffer | null> {
    try {
        const url = `https://tonapi.io/v2/tonconnect/stateinit`;
        const body = {
        ...
        const data = await response.json() as IStateInitResponseData;
...
interface IStateInitResponseData {
    public_key: string;
    address: string;
}