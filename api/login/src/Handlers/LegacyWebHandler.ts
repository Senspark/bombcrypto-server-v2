...
export class LegacyWebHandlers {
    constructor(
        private readonly _dep: IDependencies,
    ) {
        this._logger = _dep.logger.clone('[WEB]');
    ...
    public async _generateNonce(req: Request, res: Response, logger: ILogger) {
        try {
            ...
            const hash = this._botSecurity.validateUserFirstRequest(walletAddress, req, logger);
            if (!hash) {
                logger.error(`${walletAddress} Missing Client Token in generateNonce`);
    ...
    public async _checkProof(req: Request, res: Response, logger: ILogger) {
        try {
            ...
            if (!this._botSecurity.validateUserSecondRequest(walletAddress, req, logger)) {
                logger.error(`missing Server Token in checkProof`);
                return await randomResponse(res);
            }
    ...
    public async _checkProofAccount(logger: ILogger, req: Request, res: Response) {
        try {
            ...
            const hash = this._botSecurity.validateUserFirstRequest(userName, req, logger);
            if (!hash) {
    ...
    public async _refreshJwtToken(req: Request, res: Response, logger: ILogger) {
        try {
            if (!this._botSecurity.validateUserSecondRequest('<null>', req)) {
                logger.error('missing Server Token');
