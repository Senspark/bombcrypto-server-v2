...
export class RonHandlers {
    constructor(
        private readonly _dep: IDependencies,
    ) {
        this._logger = _dep.logger.clone('[RON]');
        ...
        this._loginService = new WebLoginService(
            _dep,
            RedisKeys.AP_RON_LOGIN_PRIVATE_KEY,
            RedisKeys.AP_RON_LOGIN_JWT_REFRESH,
            wallet
        );
    ...
    public async verifyLoginData(req: Request, res: Response) {
        try {
            ...
            return res.sendSuccess({
                userId: account.uid,
                walletAddress: account.address, // account ron thì ko liên quan gì đến account fi bên bsc nên luôn trả về address ko phải userName
