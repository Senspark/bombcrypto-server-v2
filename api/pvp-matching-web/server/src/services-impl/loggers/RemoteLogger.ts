...
export class RemoteLogger implements ILogger {
    ...
    constructor(
        ...
        private readonly prefix: string = '[DEFAULT]'
    ) {
    ...
    infos(...message: any[]): void {
        ...
        this._logger?.infos(m);
    }

    error(err: any): void {
        const e = `${this.prefix} ${err}`
    ...
    clone(prefix: string): ILogger {
        if (this._fallback) {
            return new RemoteLogger(this._initData, this._fallback.clone(prefix), this._zoneName, prefix);
        }
        return new RemoteLogger(this._initData, undefined, this._zoneName, prefix);
    }
}
